...
class ReReTrainer(Trainer):
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        # Create SEPARATE ConstrainedLogitsProcessor instances for test and RL generation
        # to avoid shared count state (count tracks generation step and must start at 0)
        ...
        rl_ccc = ConstrainedLogitsProcessor(
                prefix_allowed_tokens_fn=self.prefix_allowed_tokens_fn,
                num_beams=self.num_generations if self.beam_search else 1,
                base_model=self.base_model,
                eos_token_id=self.processing_class.eos_token_id
            )
        self.logits_processor = LogitsProcessorList([TemperatureLogitsWarper(temperature=self.temperature), rl_ccc])
